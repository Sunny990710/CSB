...
const USE_YN_OPTIONS = ['Y (사용)', 'N (미사용)'];
...
export default function BrandManagerView({ brands, onSave }: BrandManagerViewProps) {
  ...
  const matchesUseYn = (useYn: 'Y' | 'N') => {
    if (selectedUseYn.length === 0) return true;
    return selectedUseYn.some((opt) => opt.startsWith(useYn));
  };
  ...
  const filtered = brands.filter((b) => {
    ...
    const matchCategory = selectedCategories.length === 0 || selectedCategories.includes(b.category || '한국');
    const matchUseYn = matchesUseYn(b.useYn);
