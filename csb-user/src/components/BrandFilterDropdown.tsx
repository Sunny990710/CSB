...
import { EXAMPLE_BRANDS } from '../utils/constants';
import { getBrandFavorites, mergeBrandOptions, toggleBrandFavorite } from '../utils/brandFavorites';
...
export default function BrandFilterDropdown({ value, brands, onChange }: BrandFilterDropdownProps) {
  ...
  const toggleBrand = (brand: string) => {
    onChange(toggleMultiFilterValue(value, brand));
  };
            ...
            <ul className="max-h-64 overflow-y-auto py-1">
              <li>
                ...
                >
                  ...
                  <span className={`text-xs font-bold flex-1 ${value.length === 0 ? 'text-violet-700' : 'text-slate-800'}`}>브랜드: 전체</span>
                </button>
              </li>
